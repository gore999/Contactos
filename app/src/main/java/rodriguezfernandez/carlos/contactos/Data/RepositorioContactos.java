...
public class RepositorioContactos {
    ...
    public void deleteContacto(Contacto contacto) {
        ...
        new MyAsyncDeleteContacto(dao).execute(contacto);
    }
...
//Operaciones basicas con telefonos
    ...
    private class MyAsyncDeleteContacto extends AsyncTask<Contacto, Void, Void> {
        ContactosDao dao;

        //Constructor
        MyAsyncDeleteContacto(ContactosDao d) {
            dao = d;
        }

        @Override
        protected Void doInBackground(Contacto... contactos) {
            dao.deleteTelefonosXID(contactos[0].getId());
            dao.deleteEmailsXID(contactos[0].getId());
            dao.deleteContacto(contactos[0]);
            return null;
        }
    }
}
